...
def multitask_mean(
    ...
    nonscale_dependent_metrics = [
        ...
        "binary_cross_entropy", "sid", "wasserstein", "f1", "mcc", "recall", "precision", "balanced_accuracy", "confusion_matrix", "packet_luce_ranking"
...
def features_to_group_ids(features_batch, device, decimals=6, feat_slice=None):
    """
    Map each zeolite feature row to a small int id (0..G-1) within this batch.
    Used for grouping samples by zeolite in ranking loss.
    """
    mapping = {}
    gids = []
    next_id = 0
    for f in features_batch:
        ...
        if feat_slice is not None:
            arr = arr[feat_slice]
        key = np.round(arr, decimals=decimals).tobytes()  # stable per zeolite
        if key not in mapping:
            mapping[key] = next_id
            next_id += 1
        gids.append(mapping[key])
    return torch.tensor(gids, dtype=torch.long, device=device)