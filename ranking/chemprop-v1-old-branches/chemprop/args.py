...
Metric = Literal['auc', 'prc-auc', 'rmse', 'mae', 'mse', 'r2', 'accuracy', 'cross_entropy', 'binary_cross_entropy', 'sid', 'wasserstein', 'f1', 'mcc', 'bounded_rmse', 'bounded_mae', 'bounded_mse',
                'recall', 'precision','balanced_accuracy','packet_luce_ranking']
...
class TrainArgs(CommonArgs):
    ...
    dataset_type: Literal['regression', 'classification', 'multiclass', 'spectra', 'ranking']
    """Type of dataset. This determines the default loss function used during training."""
    loss_function: Literal['mse', 'bounded_mse', 'binary_cross_entropy', 'cross_entropy', 'mcc', 'sid', 'wasserstein', 'mve', 'evidential', 'dirichlet', 'quantile_interval','listnet'] = None
    ...
    """Temperature parameter for ListNet loss function."""
    listnet_topk: int = None
    """If specified, ListNet loss function will only consider the top k predictions for each spectrum."""
    listnet_pad_val: float = -1.0
    """Value used to pad spectra when using ListNet loss function."""
    listnet_eps: float = 1e-6
    """Small value added to the denominator of the ListNet loss function for numerical stability."""
    pytorch_seed: int = 0
    ...
    def process_args(self) -> None:
        ...
        if self.metric is None:
            if self.dataset_type == "classification":
                ...
                self.metric = "rmse"
            elif self.dataset_type == "ranking":
                self.metric = "packet_luce_ranking"
            else:
        ...
        for metric in self.metrics:
            if not any([(self.dataset_type == 'classification' and metric in ['auc', 'prc-auc', 'accuracy', 'binary_cross_entropy', 'f1', 'mcc', 'recall', 'precision', 'balanced_accuracy', 'confusion_matrix', 'soft_bce']),
                        ...
                        (self.dataset_type == 'ranking' and metric in ['packet_luce_ranking']))]):
