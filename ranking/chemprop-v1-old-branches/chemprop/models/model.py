...
class MoleculeModel(nn.Module):
    ...
    def __init__(self, args: TrainArgs):
        ...
        self.zeolite_encoder = None
        if args.use_input_features and args.features_size > 0:
            self.zeolite_encoder = nn.Sequential(
                nn.Linear(args.features_size, args.hidden_size),
                nn.ReLU(),
    ...
    def create_ffn(self, args: TrainArgs) -> None:
        ...
        if args.features_only:
            ...
            #     first_linear_dim += args.features_size
            if args.use_input_features and self.zeolite_encoder is not None:
    ...
    def forward(
        ...
        if self.is_atom_bond_targets:
            ...
            # NEW: build zeolite embedding from features_batch and concatenate
            if self.zeolite_encoder is not None and features_batch is not None:
                # features_batch is usually already a torch.Tensor from your DataLoader
                if isinstance(features_batch, list):
                    feat_tensor = torch.from_numpy(np.stack(features_batch)).float().to(encodings.device)
                else:
                    feat_tensor = features_batch.to(encodings.device).float()

                h_zeo = self.zeolite_encoder(feat_tensor)
                encodings = torch.cat([encodings, h_zeo], dim=1)
