...
from chemprop.utils import build_optimizer, build_lr_scheduler, load_checkpoint, makedirs, \
    save_checkpoint, save_smiles_splits, load_frzn_model, multitask_mean, features_to_group_ids
...
def run_training(args: TrainArgs,
    ...
    for model_idx in range(args.ensemble_size):
        ...
        # Default patience if not provided
        patience = getattr(args, "early_stopping_patience", 20)
        min_delta = getattr(args, "early_stopping_min_delta", 1e-5)
        ...
        for epoch in trange(args.epochs):
            ...
            improved = (
                (args.minimize_score and best_score - mean_val_score > min_delta) or
                (not args.minimize_score and mean_val_score - best_score > min_delta)
            )
            ...
            if improved:
                ...
                save_checkpoint(
                    ...
                    model, scaler, features_scaler,
                    atom_descriptor_scaler, bond_descriptor_scaler,
                    atom_bond_scaler, args
                )
            else:
                no_improve_count += 1
                debug(f"No improvement for {no_improve_count}/{patience} epochs")

            if no_improve_count >= patience:
                info(f"Early stopping triggered at epoch {epoch}. "
                    f"Best {args.metric} = {best_score:.6f} (epoch {best_epoch})")
                break
        ...
        # plot_violin(args, model, train_data, device=args.device)
        ...
        if empty_test_set:
            ...
            for batch in test_data_loader:
                ...
                all_test_features.extend(features_batch)

            try:
                test_group_ids = features_to_group_ids(all_test_features,device="cpu").numpy()
            except Exception as e:
                test_group_ids = None
                if logger is not None:
                    logger.debug(f"Could not compute test group IDs for per-slate ranking: {e}")
