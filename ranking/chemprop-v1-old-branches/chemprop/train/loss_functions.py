...
def contrastive_loss(zeo_emb, osda_emb, targets, margin=0.5):
    # L2 distance
    dists = torch.norm(zeo_emb - osda_emb, dim=1)
    # Contrastive loss: push/pull
    loss_pos = targets * (dists ** 2)
    loss_neg = (1 - targets) * torch.clamp(margin - dists, min=0) ** 2
...
def lambdarank_loss(preds, targets):
    """
    preds: (batch, slate_size)
    targets: (batch, slate_size), binary 0/1 relevance
    """
    batch_size, slate_size = preds.size()
    loss = 0.0
    for b in range(batch_size):
        ...
        neg_idx = torch.where(t <= 0)[0]
        if len(pos_idx) == 0 or len(neg_idx) == 0:
            continue

        pos_scores = p[pos_idx].unsqueeze(1)  # (num_pos, 1)
        neg_scores = p[neg_idx].unsqueeze(0)  # (1, num_neg)

        diff = neg_scores - pos_scores  # (num_pos, num_neg)
        pair_loss = torch.log1p(torch.exp(diff))  # log(1+exp(s_neg - s_pos))

        # Optional: weight by ΔNDCG to emphasize top ranks
        loss += pair_loss.mean()
