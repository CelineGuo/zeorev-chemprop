...
from chemprop.slate_preprocess import build_full_group_ids_from_csv, attach_slate_outputs_to_full_data, per_slate_softmax_and_rank
...
def predict_and_save(
    ...
    for i in range(len(full_data)):
        ...
        score = preds[valid_index][0] if valid_index is not None else float('-inf')  # or np.nan
        score_list.append(score)

    # 1) Build group ids from CSVs (full-data order)
    feat_slice = getattr(train_args, "group_feat_slice", None)   # must match training
    decimals   = getattr(train_args, "group_decimals", 6)

    features_path = args.features_path[0] if isinstance(args.features_path, list) else args.features_path

    full_gids, smiles_col = build_full_group_ids_from_csv(
    ...
    # 2) Compute per-slate probabilities / ranks from raw scores
    tau = getattr(args, "softmax_tau", 1.0)  # optional temperature
    prob_top1, rank_in_slate, global_rank_map = per_slate_softmax_and_rank(
        scores=score_list,
        group_ids=full_gids,
        tau=tau,
    )

    # 3) Attach columns back to rows for CSV writer
    attach_slate_outputs_to_full_data(
        full_data,
        scores=score_list,
        prob_top1=prob_top1,
        rank_in_slate=rank_in_slate,
        global_rank_map=global_rank_map,
        group_ids=full_gids,
    )


    # Step 2: Compute descending ranks (higher score = higher rank)
    sorted_indices = np.argsort(score_list)[::-1]  # descending
    rank_map = {idx: f"Rank{rank+1}" for rank, idx in enumerate(sorted_indices)}
    ...
    if save_results:
        ...
        for i, datapoint in enumerate(full_data):
            score = score_list[i]
            rank = rank_map[i]
            datapoint.row["Score"] = score
            datapoint.row["Yield / Rank"] = rank
        # Save
