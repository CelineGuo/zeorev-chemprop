...
def get_metric_func(metric: str) -> Callable[[Union[List[int], List[float]], List[float]], float]:
    ...
    * :code:`precision`: Precision
    ...
    if metric == 'packet_luce_ranking':
        return packet_luce_ranking_score
...
def packet_luce_ranking_score(targets: List[List[int]], preds: List[List[float]]) -> float:
    """
    Compute mean NDCG@k across slates (each slate may have different length),
    matching the same logic as your manual ndcg_at_k().
    """

    def dcg_at_k(relevance, k):
        """Compute Discounted Cumulative Gain (supports graded relevance)"""
        relevance = np.array(relevance)[:k]
        return np.sum((2**relevance - 1) / np.log2(np.arange(2, len(relevance) + 2)))

    def ndcg_at_k(relevance, k):
        """Compute Normalized DCG@K (graded relevance version)"""
    ...
    for t, p in zip(targets, preds):
        ...
        # Sort predictions in descending order (highest score = top rank)
        order = np.argsort(p)[::-1]
        ranked_relevance = t[order]
        k = len(ranked_relevance)

        # Compute NDCG@k for this slate
        val = ndcg_at_k(ranked_relevance, k=min(k, len(ranked_relevance)))
        ndcgs.append(val)

    # Mean NDCG across all slates
